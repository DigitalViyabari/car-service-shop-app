...
import type { Customer, CustomerType, Vehicle, VehicleFuelType, VehicleTransmission } from "@dvcs/types";
import {
  collection,
  doc,
  getDocs,
...
const emptyVehicle: VehicleDraft = {
  registrationNumber: "", make: "", model: "", variant: "", year: "", fuelType: "petrol", transmission: "manual", vin: "", odometer: "", notes: "",
};
...
export default function CustomersPage() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const [customerSnapshots, vehicleSnapshots] = await Promise.all([
  ...
  async function createVehicle(event: FormEvent) {
    ...
    try {
      ...
      batch.set(vehicleReference, {
        ...
        year: vehicleDraft.year ? Number(vehicleDraft.year) : null, fuelType: vehicleDraft.fuelType,
        ...
        <aside className="customer-detail">
          {selectedCustomer ? <>
            ...
            <div className="vehicle-list">
              {selectedVehicles.length === 0 ? <div className="vehicle-empty">No vehicle linked to this customer yet.</div> : selectedVehicles.map((vehicle) => <article className="vehicle-card" key={vehicle.id}><div className="vehicle-silhouette" aria-hidden="true">◆</div><div><strong>{vehicle.make} {vehicle.model}</strong><span>{vehicle.variant || `${vehicle.fuelType} · ${vehicle.transmission || ""}`}</span></div><div className="registration-plate">{vehicle.registrationNumber}</div><dl><div><dt>Fuel</dt><dd>{vehicle.fuelType}</dd></div><div><dt>Year</dt><dd>{vehicle.year || "—"}</dd></div><div><dt>Odometer</dt><dd>{vehicle.odometer ? `${vehicle.odometer.toLocaleString("en-IN")} km` : "—"}</dd></div></dl></article>)}
            </div>
