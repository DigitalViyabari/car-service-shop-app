...
export default function CustomersPage() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      const nextVehicles = vehicleSnapshots.docs
        .map(asVehicle)
        .filter(({ status }) => status === "active");
      ...
      setSelectedId((current) =>
        current && nextCustomers.some(({ id }) => id === current)
          ? current
  ...
  const filteredCustomers = useMemo(() => {
    ...
    const matchedCustomerIds = new Set(
      vehicles
        .filter((vehicle) => vehicle.searchRegistration.includes(registration))
        .map(({ customerId }) => customerId),
    );
    return customers.filter(
      (customer) =>
        customer.searchName.includes(term) ||
        customer.searchPhone.includes(phone) ||
  ...
  const catalogueMakes = [
    ...new Set([
      ...
      ...catalogEntries.map(({ make }) => make),
    ]),
  ].sort();
  const catalogueModels = [
    ...new Set([
      ...indianVehicleCatalogue
        .filter(({ make }) => make.toLowerCase() === vehicleDraft.make.trim().toLowerCase())
        .map(({ model }) => model),
      ...catalogEntries
        .filter(({ make }) => make.toLowerCase() === vehicleDraft.make.trim().toLowerCase())
        .map(({ model }) => model),
  ...
  const catalogueVariants = [
    ...new Set(
      catalogEntries
        .filter(
          ({ make, model }) =>
            make.toLowerCase() === vehicleDraft.make.trim().toLowerCase() &&
            model.toLowerCase() === vehicleDraft.model.trim().toLowerCase(),
        )
        .map(({ variant }) => variant)
        .filter(Boolean),
    ),
  ];
  const catalogueHasVehicle = useMemo(() => {
    const make = vehicleDraft.make.trim().toLowerCase();
    const model = vehicleDraft.model.trim().toLowerCase();
    const variant = vehicleDraft.variant.trim().toLowerCase();
    if (!make || !model) return true;
    const customMatch = catalogEntries.some(
      (entry) =>
        entry.make.toLowerCase() === make &&
        entry.model.toLowerCase() === model &&
        (!variant || (entry.variant ?? "").toLowerCase() === variant),
    );
    const defaultMatch = indianVehicleCatalogue.some(
      (entry) => entry.make.toLowerCase() === make && entry.model.toLowerCase() === model,
    );
    return customMatch || (defaultMatch && !variant);
  }, [catalogEntries, vehicleDraft.make, vehicleDraft.model, vehicleDraft.variant]);
    ...
    try {
      ...
      await writeBatch(firebaseClient.db)
        .set(reference, {
          ...
          name,
          phone,
          alternatePhone: normalizePhone(customerDraft.alternatePhone),
          email: customerDraft.email.trim().toLowerCase(),
          gstin: customerDraft.gstin.trim().toUpperCase(),
  ...
  async function saveVehicle(event: FormEvent) {
    ...
    if (registrationNumber.length < 6 || !make || !model) {
      setError("Enter a valid registration number, make, and model.");
      return;
    }
    if (
      vehicles.some(
        (vehicle) =>
          vehicle.id !== editingVehicleId && vehicle.searchRegistration === registrationNumber,
      )
    ) {
      setError("This registration number already exists in the branch.");
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      ...
      const vehicleValues = {
        ...
        colour: vehicleDraft.colour.trim(),
        year: vehicleDraft.year ? Number(vehicleDraft.year) : null,
      ...
      if (editingVehicleId) {
        batch.update(doc(firebaseClient.db, "vehicles", editingVehicleId), vehicleValues);
      } else {
        ...
        batch.update(doc(firebaseClient.db, "customers", selectedCustomer.id), {
          vehicleCount: increment(1),
          updatedAt: now,
          updatedBy: user.uid,
        });
      }
      if (saveToCatalogue && !catalogueHasVehicle) {
        batch.set(doc(collection(firebaseClient.db, "vehicleCatalog")), {
          companyId: activeCompanyId,
          make,
          model,
          variant: vehicleDraft.variant.trim(),
          bodyType: "",
          fuelTypes: [vehicleDraft.fuelType],
          transmissions: [vehicleDraft.transmission],
          yearFrom: vehicleDraft.year ? Number(vehicleDraft.year) : null,
          yearTo: vehicleDraft.year ? Number(vehicleDraft.year) : null,
          notes: "Added while registering a customer vehicle.",
          searchText: `${make} ${model} ${vehicleDraft.variant}`.toLowerCase(),
          status: "active",
          createdAt: now,
          createdBy: user.uid,
          updatedAt: now,
      ...
      setError(reason instanceof Error ? reason.message : "Unable to save vehicle changes.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function archiveCustomer() {
    if (
      ...
      !confirm(`Archive ${selectedCustomer.name}?`)
    )
      return;
    setSubmitting(true);
    try {
      await updateDoc(doc(firebaseClient.db, "customers", selectedCustomer.id), {
        status: "archived",
  ...
  return (
    <main className="content customers-page">
      ...
      {error ? (
        <div className="alert alert--error module-alert" role="alert">
          {error}
      ...
      <section className="customer-guide">
        <strong>Customer Record Guide</strong>
        <span>
          <i className="is-good" /> Vehicle linked and service-ready
        </span>
        <span>
          <i className="is-attention" /> Vehicle details still needed
        </span>
        <span>
          <i className="is-active" /> Search by name, phone or registration
        </span>
      </section>
      <section className="customer-summary">
          ...
          <strong>
            {
              customers.filter((customer) =>
                vehicles.some((vehicle) => vehicle.customerId === customer.id),
              ).length
            }
          </strong>
        </div>
        <div
          className={
            customers.some(
              (customer) => !vehicles.some((vehicle) => vehicle.customerId === customer.id),
            )
              ? "customer-amber"
              : "customer-green"
          }
          ...
          <div className="customer-list">
            ...
            {!loading && filteredCustomers.length === 0 ? (
              <div className="list-state list-state--empty">
                ...
                <strong>
                  {customers.length ? "No matching customer" : "Your customer garage is ready"}
                </strong>
                <p>
                  {customers.length
                    ? "Try a different name, phone, or registration."
                    : "Add the first customer to begin building service history."}
                </p>
              </div>
            ) : null}
              ...
              >
                ...
                <span className="vehicle-pill">
                  {customer.vehicleCount ??
                    vehicles.filter(({ customerId }) => customerId === customer.id).length}{" "}
                  vehicle{customer.vehicleCount === 1 ? "" : "s"}
                </span>
          ...
          {selectedCustomer ? (
            <>
              <div className="detail-header">
                <div className="detail-avatar">{initials(selectedCustomer.name)}</div>
                <div>
                  <span className="detail-type">{selectedCustomer.type}</span>
              ...
              <div className="contact-grid">
                ...
                <div>
                  <span>GSTIN</span>
                  <strong>{selectedCustomer.gstin || "Not provided"}</strong>
                </div>
                <div>
                  <span>Address</span>
                  <strong>{selectedCustomer.address || "Not provided"}</strong>
                </div>
              </div>
              <div className="vehicle-section-heading">
                <div>
                  <span className="heading-kicker">Garage</span>
                  <h3>Registered vehicles</h3>
                </div>
                <button onClick={openNewVehicleForm}>+ Add Vehicle</button>
              </div>
              <div
                className={`customer-readiness ${selectedVehicles.length ? "is-ready" : "needs-vehicle"}`}
              >
                <strong>
                  {selectedVehicles.length ? "Ready For Job Creation" : "Vehicle Details Needed"}
                </strong>
                <span>
                  {selectedVehicles.length
                    ? `${selectedVehicles.length} vehicle${selectedVehicles.length === 1 ? " is" : "s are"} available when creating a job card.`
                    : "Add the customer's vehicle now so reception can create a job card without leaving this screen."}
                </span>
              </div>
              <div className="vehicle-list">
                {selectedVehicles.length === 0 ? (
                  <div className="vehicle-empty">No vehicle linked to this customer yet.</div>
                ) : (
                  selectedVehicles.map((vehicle) => (
                    <article className="vehicle-card" key={vehicle.id}>
                      <div className="vehicle-silhouette" aria-hidden="true">
                        ◆
                      </div>
                      <div>
                        <strong>
                        ...
                        <span>
                          {vehicle.variant ||
                            `${fuelLabel(vehicle.fuelType)} · ${transmissionOptions.find(({ value }) => value === vehicle.transmission)?.label || ""}`}
                        </span>
                      </div>
              ...
              <div className="detail-footer">
                <button
                  ...
                  title={
                    selectedVehicles.length ? "Archive linked vehicles first" : "Archive customer"
                  }
                >
                  Archive customer
                </button>
      ...
      {showCustomerForm ? (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setShowCustomerForm(false);
          }}
        >
          <form className="module-modal" onSubmit={createCustomer}>
            <ModalHeader
              eyebrow="New Record"
              title="Add Customer"
            ...
            <div className="form-grid">
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCustomerDraft({ ...customerDraft, type: e.target.value as CustomerType })
                  }
                >
              ...
              <label>
                ...
                <input
                  inputMode="numeric"
                  value={customerDraft.alternatePhone}
                  onChange={(e) =>
                    setCustomerDraft({ ...customerDraft, alternatePhone: e.target.value })
                  }
                />
              </label>
              <label>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setCustomerDraft({ ...customerDraft, gstin: e.target.value })}
                />
              </label>
              <label className="span-2">
                ...
                <input
                  ...
                  onChange={(e) => setCustomerDraft({ ...customerDraft, address: e.target.value })}
                />
              </label>
              <label className="span-2">
                Notes
      ...
      {showVehicleForm ? (
        ...
        >
          <form className="module-modal" onSubmit={saveVehicle}>
            <ModalHeader
              eyebrow={`Vehicle Owner · ${selectedCustomer?.name ?? ""}`}
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setVehicleDraft({ ...vehicleDraft, model: e.target.value, variant: "" })
                  }
                  placeholder="Select Or Type Model"
                  required
                />
                <datalist id="vehicle-models">
                  {catalogueModels.map((value) => (
                    <option key={value} value={value} />
                  ))}
                </datalist>
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setVehicleDraft({ ...vehicleDraft, variant: e.target.value })}
                  placeholder="Select Or Type Variant"
                />
                <datalist id="vehicle-variants">
                  {catalogueVariants.map((value) => (
                    <option key={value} value={value} />
                  ))}
                </datalist>
              </label>
              <label>
                Colour
                <input
                  value={vehicleDraft.colour}
                  onChange={(e) => setVehicleDraft({ ...vehicleDraft, colour: e.target.value })}
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setVehicleDraft({
                      ...vehicleDraft,
                      fuelType: e.target.value as VehicleFuelType,
                    })
                  }
                >
              ...
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={vehicleDraft.odometer}
                  onChange={(e) => setVehicleDraft({ ...vehicleDraft, odometer: e.target.value })}
                />
              </label>
              <label className="span-2">
                VIN / Chassis Number
              ...
              <label className="span-2">
                ...
                <textarea
                  ...
                  onChange={(e) => setVehicleDraft({ ...vehicleDraft, notes: e.target.value })}
                  rows={3}
                />
              </label>
              {!catalogueHasVehicle ? (
                <label className="span-2 inline-catalogue-option">
                  <input
                    ...
                    onChange={(event) => setSaveToCatalogue(event.target.checked)}
                  />
                  <span>
                    <strong>Save To My Company Vehicle List</strong>
                    <small>
                      This Make, Model and Variant will appear as an option for future customers.
                    </small>
                  </span>
                </label>
              ) : null}
            ...
            <ModalFooter
              ...
              label={editingVehicleId ? "Save Changes" : "Add Vehicle"}
            />
          </form>
        </div>
      ) : null}
...
function ModalHeader({
  ...
  return (
    <header className="modal-header">
      <div>
        <span className="heading-kicker">{eyebrow}</span>
        <h2>{title}</h2>
      </div>
...
function ModalFooter({
  ...
  return (
    <footer className="modal-footer">
      ...
      <button className="dv-button" type="submit" disabled={submitting}>
